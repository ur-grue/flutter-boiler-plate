...
- Platform folders (`android/`, `ios/`, …) — generated by `flutter create .`. **Only ever run
  `flutter create .` from the repo root** — running it in a subfolder scaffolds a nested counter
  app whose `pubspec.yaml` hijacks the IDE/run. Never create or commit a second `pubspec.yaml`.
